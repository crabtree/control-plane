...
func NewUpgradeKymaStep(os storage.Operations, runtimeStorage storage.RuntimeStates, cli provisioner.Client, log logrus.FieldLogger, timeSchedule *TimeSchedule) *UpgradeKymaStep {
	ts := timeSchedule
	if ts == nil {
		ts = &TimeSchedule{
			Retry:              5 * time.Second,
...
func (s *UpgradeKymaStep) Run(operation internal.UpgradeKymaOperation, opLog logrus.FieldLogger) (internal.UpgradeKymaOperation, time.Duration, error) {
	...
	opLog.Infof("kyma upgrade process initiated successfully")
