...
func Test_Deprovision_RetryOperationOnce(t *testing.T) {
	...
	opManager := NewDeprovisionOperationManager(operations, fixLogger())
...
func Test_Deprovision_RetryOperationWithoutFail(t *testing.T) {
	...
	// then - first call
	op, when, err := opManager.RetryOperationWithoutFail(op, errorMessage, retryInterval, retryInterval+1)
